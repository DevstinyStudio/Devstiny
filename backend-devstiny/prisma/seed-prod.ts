/**
 * Production seed — essential data only.
 * Safe to run on a fresh production database.
 *
 * Does NOT:
 *   - delete any existing data
 *   - create dummy users or forum threads
 *   - create an admin user (use create-admin.ts for that)
 *
 * Run with:
...
async function main() {
  ...
  const questData = [
    {
      ...
      testCases: [{ description: "Separates heavy (>5kg) and light (≤5kg)", args: [[{ name: "Warhammer", weight: 8 }, { name: "Dagger", weight: 1.2 }]], expected: { heavy: ["Warhammer"], light: ["Dagger"] } }],
    ...
    },
    {
      ...
      testCases: [{ description: "Sorts orders by waitDays descending", args: [[{ name: "Axe", waitDays: 3 }, { name: "Sword", waitDays: 7 }]], expected: ["Sword", "Axe"] }],
    ...
    },
    {
      ...
      testCases: [{ description: "Removes duplicates and zero-stock items", args: [[{ name: "Iron", qty: 5 }, { name: "Iron", qty: 5 }, { name: "Coal", qty: 0 }]], expected: ["Iron"] }],
    ...
    },
    {
      ...
      testCases: [{ description: "Counts each transformation type", args: [["fire", "ice", "fire", "wind"]], expected: { fire: 2, ice: 1, wind: 1 } }],
    ...
    },
    {
      ...
      testCases: [{ description: "Returns the material with largest gap", args: [[{ name: "Iron", pure: 10, corrupted: 3 }, { name: "Gold", pure: 20, corrupted: 18 }]], expected: "Iron" }],
    ...
    },
    {
      ...
      testCases: [{ description: "Returns methods sorted by frequency", args: [[["fire", "ice"], ["fire", "wind"], ["fire"]]], expected: ["fire", "ice", "wind"] }],
    ...
    },
    {
      ...
      testCases: [{ description: "Returns residents over 40 within 5km", args: [[{ name: "Ada", age: 45, distance: 3 }, { name: "Bob", age: 30, distance: 2 }]], expected: ["Ada"] }],
    ...
    },
    {
      ...
      testCases: [{ description: "Sums only non-null values", args: [[100, null, 200, null, 50]], expected: 350 }],
      rewardXp: 150, rewardGold: 120, rewardBadge: "Auditor", isActive: true,
    },
    {
      ...
      testCases: [{ description: "Identifies accounts that go negative", args: [[{ name: "Ada", transactions: [100, -150, 50] }, { name: "Bob", transactions: [200, -100] }]], expected: ["Ada"] }],
  ...
  await prisma.forumCategory.createMany({
    skipDuplicates: true,
    data: [
      { slug: "tavern",           title: "The Tavern",           description: "General talk. Introduce yourself. Share what you've been learning.",                     gem: "/gem/gem-6.png",  color: "text-rpg-gold",   order: 1 },
      { slug: "oracle",           title: "The Oracle",           description: "Ask anything about quests, the Books, or your adventure. No question is too small.",     gem: "/gem/gem-14.png", color: "text-rpg-cyan",   order: 2 },
      { slug: "hall-of-champions", title: "Hall of Champions",   description: "Show your projects, get feedback, inspire others.",                                      gem: "/gem/gem-22.png", color: "text-rpg-green",  order: 3 },
      { slug: "guild-board",      title: "Guild Board",          description: "Looking for study partners? Form a guild or recruit new members.",                       gem: "/gem/gem-33.png", color: "text-rpg-purple", order: 4 },
    ],
  });
  ...
  // ── Act content from JSON files (if present) ───────────────────────────────
  ...
  // ── Migrate BookChapter sections ───────────────────────────────────────────
  const allChapters = await prisma.bookChapter.findMany();
  const toMigrate = allChapters.filter((ch) => ch.sections === null);
  ...
  if (toMigrate.length > 0) console.log(`✅ Migrated ${toMigrate.length} book chapters to sections format.`);

  console.log("\n✅ Production seed complete. No dummy users created.");
  console.log("   → Run  npx ts-node --esm prisma/create-admin.ts  to create the admin account.");
}
